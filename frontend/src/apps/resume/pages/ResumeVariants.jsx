    ...
    <div>
      ...
      {resumes.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {resumes.map(r => (
            <Card key={r._id} className="flex flex-col group">
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-comic-yellow border-2 border-ink text-ink flex items-center justify-center shrink-0">
                  <FileText size={18} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-ink dark:text-white truncate">{r.variantName}</span>
                    {r.isDefault && <Badge color="amber" className="gap-1"><Star size={11} className="fill-current" /> Default</Badge>}
                  </div>
                  <p className="text-sm text-ink/60 dark:text-white/60 truncate font-sans">{r.name || 'Unnamed'} — {r.headline || 'No headline'}</p>
                  <p className="text-xs text-ink/50 dark:text-white/50 mt-1 capitalize font-sans">{r.template} template</p>
                </div>
              </div>
              <div className="flex items-center gap-2 mt-4 pt-4 border-t-2 border-ink/15 dark:border-white/15">
